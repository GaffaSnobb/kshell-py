...
def main():
    ...
    for idx_left in range(m_dim):
        for idx_right in range(m_dim):
            ...
            for a in range(3):  # There are 3 orbitals in the model space.
                b = a   # Most generally, should be for b in range(3), but the SPEs are only defined for a = b.
    ...
    j_idx_to_j = [3, 5, 1] # 0 is 3/2, 1 is 5/2, 0 is s1/2
    for idx_left in range(m_dim):
        for idx_right in range(m_dim):
